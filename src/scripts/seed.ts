...
const startOfDayUtc = (daysAgo: number): Date => {
  const date = new Date();
...
const main = async (): Promise<void> => {
  ...
  try {
    const password = await bcrypt.hash(QA_USER.password, 10);
    ...
    const existingLinks = await Url.find({ userId: user._id, shortId: { $in: linkFixtures.map(({ shortId }) => shortId) } });
    if (existingLinks.length) {
      await Click.deleteMany({ urlId: { $in: existingLinks.map(({ _id }) => _id) } });
      await Url.deleteMany({ _id: { $in: existingLinks.map(({ _id }) => _id) } });
    }
    ...
    const links = await Promise.all(linkFixtures.map(async (fixture, index) => {
      ...
      return {
        ...
        userId: user._id,
        qrCodeDataUrl: await QRCode.toDataURL(shortUrl, { errorCorrectionLevel: 'H', width: 300, margin: 2 }),
        createdAt: startOfDayUtc(10 + index),
        updatedAt: new Date(),
      };
    }));
    const seededLinks = await Url.insertMany(links);
    ...
    await Promise.all(seededLinks.map((link) => Url.updateOne(
      { _id: link._id },
      { $set: { clicks: clickCounts.get(link._id.toString()) || 0 } },
    )));
