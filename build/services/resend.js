...
exports.sendTicketEmail = exports.sendEventNoticeEmail = exports.createTicketEmailTemplate = void 0;
const qrcode_1 = __importDefault(require("qrcode"));
const escapeHtml = (value) => value.replace(/[&<>'"]/g, (character) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' }[character] || character));
const createTicketEmailTemplate = (input) => {
...
exports.createTicketEmailTemplate = createTicketEmailTemplate;
const sendEventNoticeEmail = async (input) => {
    ...
    if (!response.ok)
        throw new Error(payload.message || 'Unable to send event notification');
    return payload.id;
};
exports.sendEventNoticeEmail = sendEventNoticeEmail;
const sendTicketEmail = async (input) => {
    const apiKey = process.env.RESEND_API_KEY;
    if (!apiKey) {
    ...
    const from = process.env.RESEND_FROM_EMAIL || 'Shortly Events <onboarding@resend.dev>';
    const qrPng = (await qrcode_1.default.toBuffer(input.ticketUrl, { width: 420, margin: 2, errorCorrectionLevel: 'H' })).toString('base64');
    const template = (0, exports.createTicketEmailTemplate)(input);
    const response = await fetch('https://api.resend.com/emails', {
        method: 'POST',
    ...
    const payload = await response.json();
    if (!response.ok)
        throw new Error(payload.message || 'Unable to send the registration email');
    return payload.id;
};
